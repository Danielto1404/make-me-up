...
from ..api.model import GenerateMakeRequestParams
...
class CLIPTrainer:
    ...
    def train(self, params: GenerateMakeRequestParams):
        targets = [self.clip_model.embed_text(text) for text in params.prompts]

        initial = self._initial_search(
            targets,
            iterations=params.initial_iterations,
            batch_size=params.batch_size,
            truncation_psi=params.truncation_psi
        )

        latent = self._optimize(initial, targets, iterations=params.iterations)
